...
async def test_user_password():
    ...
    async with async_session() as session:
        ...
        for u in users:
            ...
            print(
                f"ID: {u.id} | Email: {u.email} | Superuser: {u.is_superuser} | Verified: {u.is_verified} | Hashed PW: {u.hashed_password}"
            )
            print(
                f"  -> Match 'userpassword'?: {match_p1} | Match 'adminpassword'?: {match_p2}"
            )
